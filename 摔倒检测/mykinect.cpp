...
HRESULT CBodyBasics::InitializeDefaultSensor()
{
	...
	//搜索kinect 
	hr = GetDefaultKinectSensor(&m_pKinectSensor);  //获取Kinect传感器
	if (FAILED(hr))
	{
		return hr;
	}
	...
	if (m_pKinectSensor)
	{
	    ...
	    //打开Kinect
		hr = m_pKinectSensor->Open();    //判断Kinect是否正常打开

		//coordinatemapper
		if (SUCCEEDED(hr))         //如果正常打开了
		{
			hr = m_pKinectSensor->get_CoordinateMapper(&m_pCoordinateMapper);    //判断坐标映射是否正常
		}

		//bodyframe    每种图都是通过source，reader，frame三个类
		if (SUCCEEDED(hr))
		{
			hr = m_pKinectSensor->get_BodyFrameSource(&pBodyFrameSource);  //获取骨架数据信息  
		}

		if (SUCCEEDED(hr))
		{
			hr = pBodyFrameSource->OpenReader(&m_pBodyFrameReader);      //读取骨架序列信息
		}
		//color frame
		if (SUCCEEDED(hr))
		{
			hr = m_pKinectSensor->get_ColorFrameSource(&pColorFrameSource); //获取彩色数据信息
		}

		if (SUCCEEDED(hr))
		{
			hr = pColorFrameSource->OpenReader(&m_pColorFrameReader);    //打开彩色数据信息
		}

		//depth frame
		if (SUCCEEDED(hr))
		{
			hr = m_pKinectSensor->get_DepthFrameSource(&pDepthFrameSource);   // 获取深度信息
		}

		if (SUCCEEDED(hr))
		{
			hr = pDepthFrameSource->OpenReader(&m_pDepthFrameReader);        //读取深度信息
		}

		//body index frame
		if (SUCCEEDED(hr))                                                    
		{
			hr = m_pKinectSensor->get_BodyIndexFrameSource(&pBodyIndexFrameSource); //获得二值化背景图
		}

		if (SUCCEEDED(hr))
		{
			hr = pBodyIndexFrameSource->OpenReader(&m_pBodyIndexFrameReader);     //读取二值化背景图
		}

		SafeRelease(pBodyFrameSource);                               //释放指向COM口的指针
		SafeRelease(pDepthFrameSource);
		SafeRelease(pColorFrameSource);
		SafeRelease(pBodyIndexFrameSource);
	}

	if (!m_pKinectSensor || FAILED(hr))                         //kinect 初始化失败
	{
		...
		if (pBody)//还没有搞明白这里pBody和下面的bTracked有什么区别
		{
			...
			if (SUCCEEDED(hr) && bTracked)
			{
				...
				/****************************PPT动作控制******************************************/
					...
					cout << "头的高度" << joints[JointType_Head].Position.Y << endl;
					cout << "右脚的高度"<< joints[JointType_FootLeft].Position.Y << endl;
					cout << "左脚的高度" << joints[JointType_FootRight].Position.Y << endl;
					cout << "第几副骨架" <<i<< endl;
